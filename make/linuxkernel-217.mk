...
$(BUILD_TMP)/linux-$(KVERSION_FULL):
	...
	rm -rf linux-sh4*
	[ -d "$(STL_ARCHIVE)/linux-sh4-2.6.32.y.git" ] && \
	(cd $(STL_ARCHIVE)/linux-sh4-2.6.32.y.git; git pull; cd "$(buildprefix)";); \
	[ -d "$(STL_ARCHIVE)/linux-sh4-2.6.32.y.git" ] || \
	git clone git://git.stlinux.com/stm/linux-sh4-2.6.32.y.git $(STL_ARCHIVE)/linux-sh4-2.6.32.y.git;protocol=git;branch=stmicro; \
		rm -fr $(TMP_KDIR)
		cp -ra $(STL_ARCHIVE)/linux-sh4-2.6.32.y.git $(BUILD_TMP)/linux-2.6.32; \
			set -e; cd $(TMP_KDIR); \
